...
class WalletGoalController extends AbstractController
{
    ...
    #[Route('/wallet-goals/analyze/{id}', name: 'wallet_goal_analyze', methods: ['GET'])]
    public function analyze(WalletGoal $goal, Request $request): JsonResponse
    {
        ...
        $data = [
            ...
            'current_balance' => $goal->getWallet()->getBalance(),
            'progression' => $goal->getProgression(),
            'deadline' => $goal->getDeadline()->format('Y-m-d'),
            'days_remaining' => (new \DateTime())->diff($goal->getDeadline())->days
        ];

        $advice = $this->aiService->getFinancialAdvice($data, 'Dynamic');
